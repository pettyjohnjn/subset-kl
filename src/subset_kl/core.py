...
from .base import ReductionType, apply_reduction
...
def select_head_tail_indices(
    ...
    """
    Select a deterministic top-k head and Monte Carlo samples from the tail.
...
def subset_kl_from_gathered_with_weights(
    ...
    """
    Compute importance-weighted KL (Frankenstein estimator) from pre-gathered tensors.
    ...
    """
    from .sampling import frankenstein_kl_estimate

    B, T, S = student_logits_k.shape

    # Flatten for Frankenstein computation
    ...
    kl_flat, _ = frankenstein_kl_estimate(
...
def subset_k2_kl_from_gathered(
    ...
    """
    Compute top-k head KL plus a Monte Carlo K2 penalty on sampled tail tokens.
...
def subset_mc_kl_from_gathered(
    student_logits_selected: torch.Tensor,
    teacher_log_probs_selected: torch.Tensor,
    k_head: int,
    k_tail: int,
    ...
    """
    Compute exact head KL plus a Monte Carlo estimate of the teacher-tail KL.
    ...
    if student_log_normalizer is None:
        raise ValueError(
            "student_log_normalizer is required because the Monte Carlo estimator "
...
def _tail_estimator_diagnostics(
    ...
    tail_estimate: torch.Tensor | None,
...
def subset_hajek_kl_from_gathered(
    student_logits_selected: torch.Tensor,
    teacher_log_probs_selected: torch.Tensor,
    k_head: int,
    k_tail: int,
    ...
    """
    Compute exact head KL plus a proper self-normalized Hajek tail estimate.

    The head is the exact full-vocabulary KL contribution for the deterministic
    top-k tokens. The tail estimates the teacher-tail expectation using
    self-normalized importance sampling:

        (1 - P_head) * sum_j w_j f_j / sum_j w_j

    where ``f_j = log P(t_j) - log Q(t_j)`` and
    ``w_j = P_tail(t_j) / proposal(t_j)``. When tail samples come from
    ``select_head_tail_indices()``, the proposal is the teacher tail itself and
    this reduces to the plain Monte Carlo estimator.
    """
    if k_head < 0 or k_tail < 0:
        raise ValueError("k_head and k_tail must be non-negative")
    if k_head + k_tail == 0:
    ...
    if student_log_normalizer is None:
        raise ValueError(
            "student_log_normalizer is required because the Hajek estimator "
            "uses full-vocabulary student log-probabilities"
        )
    if student_log_normalizer.shape != leading_shape:
        raise ValueError("student_log_normalizer shape must match leading dimensions")
    ...
    if k_tail > 0:
        ...
        if tail_proposal_log_probs_selected is None:
            proposal_tail = teacher_tail - tail_mass.unsqueeze(-1).log()
        else:
            if tail_proposal_log_probs_selected.shape != teacher_tail.shape:
                raise ValueError(
                    "tail_proposal_log_probs_selected shape must match tail selected shape"
                )
            proposal_tail = tail_proposal_log_probs_selected

        log_teacher_tail_conditional = teacher_tail - tail_mass.unsqueeze(-1).log()
        weights = (log_teacher_tail_conditional - proposal_tail).exp()
        weighted_tail = (weights * tail_terms).sum(dim=-1)
        weight_sum = weights.sum(dim=-1).clamp_min(1e-12)
        tail_estimate = tail_mass.detach() * weighted_tail / weight_sum
        per_position = per_position + tail_estimate

    loss = apply_reduction(per_position, attention_mask, reduction)
    if return_diagnostics:
        diagnostics = _tail_estimator_diagnostics(
            weights=weights if k_tail > 0 else None,
            tail_estimate=tail_estimate if k_tail > 0 else None,
            total_estimate=per_position,
            attention_mask=attention_mask,
        )
...
def compute_subset_mc_kl(
    ...
    """
    Compute exact head plus Monte Carlo teacher-tail KL when full logits are available.
...
def compute_subset_hajek_kl(
    ...
    """
    Compute exact head plus self-normalized Hajek teacher-tail KL.

    With the default ``select_head_tail_indices()`` sampler, the proposal is the
    teacher tail distribution, so this equals the Monte Carlo estimator. The
    gathered API accepts explicit proposal log-probabilities for non-teacher
    tail proposals.
    """
    use_explicit_proposal = tail_proposal not in {"target", "teacher"}
    selected = select_head_tail_indices(
